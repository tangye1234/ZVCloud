...
public class HttpImageManager{
    ...
    public static final int DECODING_MAX_PIXELS_DEFAULT = Integer.MAX_VALUE;
    ...
    private Runnable newRequestCall(final LoadRequest request, final boolean isManaged) {
        return new Runnable() {
            ...
            public void run() {
            	if (DEBUG) Log.d(TAG, "run for request " + request.getUri().toString());
                ...
                synchronized (mActiveRequests) {
                    ...
                    while (mActiveRequests.contains(request)) {
                        try {
                        	if (DEBUG) Log.d(TAG, "wait for request " + request.getUri().toString());
                        ...
                        synchronized ( iv ) {
                            if ( iv.getTag() == request.getUri() ) {
                                mHandler.post(new Runnable() {
                                    ...
                                    public void run() {
                                        if ( iv.getTag() == request.getUri()) {
                                        	OnLoadResponseListener r = request.mListener;
                                        	if (r != null) {
                                        		if (!r.onBeforeSetImageBitmap(iv, finalData)) {
                                        			return;
                                        		}
                                        	}
